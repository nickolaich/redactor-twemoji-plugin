    ...
    $.Redactor.prototype.twemoji = function(){
        return {
            ...
            show: function(){
                ...
                modal.addTemplate('twemoji', _this.twemoji.getTemplate());
                modal.load('twemoji', _this.lang.get('twemoji'), 700);
                modal.show();

                $twemojiSection = $('#redactor-twemoji-section');

                //modal.createTabber($twemojiSection);
                //modal.getModal().append($twemojiSection);

                for(setName in _this.opts.emojiset){
                    var twemojiSet = _this.opts.emojiset[setName];
                    if(twemojiSet.length){
                        setIndex++;
                        var $setTab = $('<div id="redactor-image-manager-box" style="overflow: auto; height: 300px;" class="redactor-modal-tab redactor-tab redactor-tab' + setIndex + '"></div>');
                        $setTab.attr('data-title', setName);
                        if(setIndex == 1){
            ...
            addButton: function(){
                var _this  = this,
                    button = _this.button.add(_this.twemoji.pluginName, _this.lang.get('twemoji'));
                if (_this.opts.toolbaricon){
                    var img = $("<img />");
                    img.prop('src', _this.opts.toolbaricon);
                    img.css('width', _this.opts.toolbariconwidth);
                    button.html(img);
                } else {
            ...
            insert: function(e){
                ...
                if(emoji !== ''){
                    ...
                    imageNode.src = emoji.attr('src');
                    imageNode.className = emoji.attr('class');

                    var range = _this.selection.range();
                    range.insertNode(imageNode);
                    _this.selection.restore();
                    _this.core.callback('insertedEmoji', emoji);
